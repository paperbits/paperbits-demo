...
define('events',[
	...
	function kill(doc, editor) {
	    remove(doc, 'resize', editor);

	    remove(doc, 'keyup', editor);
	    remove(doc, 'keydown', editor);
	    remove(doc, 'keypress', editor);

	    remove(doc, 'click', editor);
	    remove(doc, 'mouseup', editor);
	    remove(doc, 'mousedown', editor);
	    remove(doc, 'mousemove', editor);
	    remove(doc, 'dblclick', editor);

	    remove(doc, 'dragstart', editor);
	    remove(doc, 'drag', editor);
	    remove(doc, 'dragenter', editor);
	    remove(doc, 'dragexit', editor);
	    remove(doc, 'dragleave', editor);
	    remove(doc, 'dragover', editor);
	    remove(doc, 'drop', editor);
	    remove(doc, 'dragend', editor);
	    remove(doc, 'paste', editor);
	}
...
define('ui',[
	...
	var commands = {
		...
		'unformat'  : {
			...
			'nodes'  : ['b', 'i', 'u', 'em', 'strong', 'sub', 'sup', 'del', 'small', 'code', 'a']
		}
	};
...
define('api',[
	...
	exports['events']['kill']            = Events.kill;
...
define('alohaeditor',[
	...
	Api['mahalo'] = mahalo;
	Api['editor'] = editor;
	Api['buildcommit'] = 'https://github.com/alohaeditor/Aloha-Editor/commit/HEAD';
